...
export default function InsightPage() {
  ...
  const [text, setText] = React.useState<string>("Run to generate insights from Dashboard data.");
  ...
  async function run() {
    ...
    setText("Generating…");
    try {
      const r = await fetch("/api/insight/run", {
        method: "POST",
      ...
      setText(j.summary || "No insights.");
      setDataBlob(j.data || null);
    } catch (e: any) {
  ...
  // super-light local Q&A over returned arrays (no external LLM)
  function ask() {
    if (!dataBlob || !q.trim()) return;
    const qq = q.toLowerCase();
    if (qq.includes("top page") || qq.includes("top pages")) {
      const top = (dataBlob.ga?.topPages ?? []).slice(0, 5).map((p: any) => `${p.path} (${p.sessions})`).join("; ");
      setText(`Top GA pages: ${top || "n/a"}`);
      return;
    }
    if (qq.includes("top quer")) {
      ...
      setText(`Top GSC queries: ${top || "n/a"}`);
      return;
    }
    if (qq.includes("trend") || qq.includes("increase") || qq.includes("decrease")) {
      const s = dataBlob.ga?.series ?? [];
      const change = s.length > 7 ? ((s.at(-1)?.sessions - s.at(-8)?.sessions) / Math.max(1, s.at(-8)?.sessions)) * 100 : 0;
      setText(`Last-week vs prev-week session change ≈ ${isFinite(change) ? change.toFixed(1) : "0"}%`);
      return;
    }
  ...
  if (!session)
    return (
      <main className="p-6">
        ...
        <p>Sign in to generate insights.</p>
      </main>
    );
