...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    for (const row of gsc.rows || []) {
      const d = row.keys?.[0];
      if (!d) continue;
      clicksByDate[d] = (clicksByDate[d] || 0) + Number(row.clicks || 0);
      impressionsByDate[d] = (impressionsByDate[d] || 0) + Number(row.impressions || 0);
    ...
    const series = dates.map(d => ({
      ...
      clicks: clicksByDate[d] || 0,
      impressions: impressionsByDate[d] || 0,
      ctr: impressionsByDate[d] ? (clicksByDate[d] || 0) / impressionsByDate[d] : 0,
